        ...
        for i in repeat_data:
            ...
            if len(group_indices) > Knn:
                if Knn == 1 and len(group_indices) == 2:
                    ...
                    nn_index_X[i, :] = random.sample(
                        [x for x in group_indices if x != i], Knn
                    )
            else:
                if (
                    distances.shape[1] > Knn + 2
                    and distances[i, Knn + 1] < distances[i, Knn + 2]
                ):
                    ...
                    id_equal = random.sample(
                        list(np.where(dist_matrix == tie_dist)[0]), Knn - len(id_small)
                    )
